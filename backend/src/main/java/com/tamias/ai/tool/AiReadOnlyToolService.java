...
public class AiReadOnlyToolService {
    ...
    public AiToolAnswer indexedDocuments() {
        List<Map<String, Object>> rows = documentRows(null,
                " AND EXISTS (SELECT 1 FROM document_chunks dcx WHERE dcx.document_id = d.id AND dcx.organization_id = d.organization_id AND dcx.vector_store_id IS NOT NULL) ",
                null, DEFAULT_LIMIT, "d.created_at DESC");
        return documentRowsAnswer(rows, "document.indexed", "Estos documentos tienen evidencia de indexación IA:", "No encontré documentos indexados para IA.");
    }
    ...
    public AiToolAnswer notIndexedDocuments() {
        List<Map<String, Object>> rows = documentRows(null,
                ...
                null, DEFAULT_LIMIT, "d.created_at DESC");
        return documentRowsAnswer(rows, "document.notIndexed", "Estos documentos todavía no tienen evidencia de indexación IA:", "No encontré documentos pendientes de indexación IA.");
    }
    ...
    public AiToolAnswer documentCountByType() {
        ...
        List<Map<String, Object>> rows = query("""
                ...
                """, q -> {
                    q.setParameter("organizationId", organizationId);
                    q.setParameter("limit", DEFAULT_LIMIT);
                }, "documentType", "documentCount", "processedCount", "indexedCount");
        if (rows.isEmpty()) {
        ...
        StringBuilder answer = new StringBuilder("Este es el conteo de documentos por tipo:");
        for (Map<String, Object> row : rows) {
            answer.append(System.lineSeparator())
                    ...
                    .append(" | indexados: ").append(blankToDash(value(row.get("indexedCount"))));
        }
    ...
    public AiToolAnswer documentCountByProperty() {
        ...
        List<Map<String, Object>> rows = query("""
                ...
                """, q -> {
                    q.setParameter("organizationId", organizationId);
                    q.setParameter("limit", DEFAULT_LIMIT);
                }, "propertyName", "documentCount", "processedCount", "indexedCount");
        if (rows.isEmpty()) {
        ...
        StringBuilder answer = new StringBuilder("Este es el conteo de documentos por propiedad:");
        for (Map<String, Object> row : rows) {
            answer.append(System.lineSeparator())
                    .append("- ").append(blankToDash(value(row.get("propertyName"))))
                    .append(" | documentos: ").append(blankToDash(value(row.get("documentCount"))))
