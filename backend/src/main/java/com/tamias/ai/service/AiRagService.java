...
public class AiRagService {
    ...
    public AiChatResponse chat(AiChatRequest request) {
        ...
        if (matches.isEmpty()) {
            String fallbackAnswer = "No encontré información relacionada en los documentos indexados. Puedes intentar bajar el similarityThreshold o indexar documentos más específicos.";
            AiChatMessage assistantMessage = chatSessionService.saveMessage(session, AiChatMessageRole.ASSISTANT, fallbackAnswer);
            return new AiChatResponse(session.getId(), userMessage.getId(), assistantMessage.getId(),
                    request.question(), fallbackAnswer, false, 0, sources);
        }
        ...
        return new AiChatResponse(session.getId(), userMessage.getId(), assistantMessage.getId(),
                request.question(), answer, true, sources.size(), sources);
    }

    private List<org.springframework.ai.document.Document> searchSimilarDocuments(String question, UUID propertyId, Integer topK, Double similarityThreshold) {
    ...
    private String buildFilterExpression(UUID propertyId) {
        ...
        return "%s == '%s' && %s == '%s'".formatted(
                RagMetadataKeys.ORGANIZATION_ID, organizationId, RagMetadataKeys.PROPERTY_ID, propertyId.toString());
    }
    ...
    private AiSourceResponse toSourceResponse(int sourceNumber, org.springframework.ai.document.Document document) {
        Map<String, Object> metadata = document.getMetadata();
        return new AiSourceResponse("S" + sourceNumber, document.getId(),
                ...
                document.getScore(), buildExcerpt(document.getText()));
    }
